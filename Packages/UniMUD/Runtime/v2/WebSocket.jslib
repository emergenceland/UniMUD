...
var LibraryWebSocket = {
   ...
   *
   * @param instanceId Instance ID
   * @param type Pointer to subscription type string (e.g., "newHeads", "logs", etc.)
   */
  WebSocketSendJsonRpcSubscribe: function (instanceId, type) {
    var instance = webSocketState.instances[instanceId];
    if (!instance) return -1;
    ...
    // Create JSON-RPC subscription request
    ...
    instance.ws.send(JSON.stringify(subscriptionRequest));
