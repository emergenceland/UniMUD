...
ResourceId constant _tableId = ResourceId.wrap(bytes32(abi.encodePacked(RESOURCE_TABLE, bytes14(""), bytes16("Eth"))));
ResourceId constant EthTableId = _tableId;
...
library Eth {
   ...
   */
  function getValue(IStore _store, bytes32 key) internal view returns (uint256 value) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = key;
