...
ResourceId constant _tableId = ResourceId.wrap(bytes32(abi.encodePacked(RESOURCE_TABLE, bytes14(""), bytes16("Toad"))));
ResourceId constant ToadTableId = _tableId;
...
library Toad {
   ...
   */
  function getValue(IStore _store, bytes32 key) internal view returns (bool value) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = key;
   ...
   */
  function setValue(IStore _store, bytes32 key, bool value) internal {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = key;
